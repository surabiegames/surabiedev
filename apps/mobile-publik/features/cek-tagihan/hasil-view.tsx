...
function KartuPelanggan({ pelanggan }: { pelanggan: CustomerInfo }) {
  return (
    <Card
      ...
      description={`No. Langganan ${pelanggan.nomorLangganan}`}
      trailing={pelanggan.status ? <Badge variant="outline">{pelanggan.status}</Badge> : undefined}
    >
...
function KartuTunggakan({ total }: { total: number }) {
  ...
  return (
    <Card
      ...
      title={lunas ? 'Tidak Ada Tunggakan' : 'Total Tunggakan'}
      description={
        lunas
...
function KartuTagihan({ tagihan: t }: { tagihan: BillModel }) {
  ...
  return (
    <Card
      title={t.labelPeriodeTagihan}
      description={t.pemakaianM3 == null ? 'Pemakaian -' : `Pemakaian ${formatM3(t.pemakaianM3)}`}
      trailing={
