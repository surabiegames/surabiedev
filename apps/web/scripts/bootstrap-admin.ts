// apps/web/scripts/bootstrap-admin.ts — membuat akun SUPER_ADMIN pertama.
...
// LOKASI: script ini ada di apps/web (bukan packages/db) karena butuh
// `hashPassword` dari @workspace/auth, sementara @workspace/auth sendiri
// bergantung pada @workspace/db — menaruhnya di packages/db akan membuat
// dependensi melingkar. apps/web sudah bergantung ke dua-duanya.
//
// CATATAN: script ini WAJIB dijalankan lewat `pnpm bootstrap-admin` dari
// apps/web, yang memakai `tsx --conditions=react-server`. Sebabnya
// @workspace/auth/password mengimpor
...
// Menjalankan `npx tsx scripts/bootstrap-admin.ts` tanpa flag itu akan gagal.
import "./env"
